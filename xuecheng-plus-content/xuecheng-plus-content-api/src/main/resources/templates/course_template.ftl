...
<head>
    ...
    <title>学成在线-${model.course.name}</title>
...
<div class="article-banner">
    ...
    <div class="banner-info">
        <div class="banner-left">
            <p>${model.course.mtName!''}<span>\ ${model.course.stName!''}</span></p>
            <p class="tit">${model.course.name}</p>
            <p class="pic">
                <#if model.course.charge=='201000'>
                    <span class="new-pic">免费</span>
                <#else>
                    <span class="new-pic">特惠价格￥${model.course.price!''}</span>
                    <span class="old-pic">原价￥${model.course.originalPrice!''}</span>
                </#if>
            </p>
            <p class="info">
                <a href="#" @click.prevent="startLearning()">马上学习</a>
                <span><em>难度等级</em>
                <#if model.course.grade=='204001'>
                    初级
                 <#elseif model.course.grade=='204002'>
                    中级
                <#elseif model.course.grade=='204003'>
                    高级
                </#if>
                </span>
                ...
                <span><em>授课模式</em>
                 <#if model.course.teachmode=='200002'>
                     录播
                 <#elseif model.course.teachmode=='200003'>
                     直播
                 </#if>
                </span>
            </p>
        ...
        <div class="banner-rit">
            <p>
                <a href="http://www.51xuecheng.cn/course/preview/learning.html?id=${model.course.id}" target="_blank">
                    <#if model.course.pic??>
                        <img src="http://file.51xuecheng.cn${model.course.pic}" alt="" width="270" height="156">
                    <#else>
...
<div class="article-cont">
                                ...
                                <div class="drop-down" style="height: 260px;">
                                    <ul class="list-box">
                                        <#list firstNode.teachPlanTreeNodes as secondNode>
                                            <li><a href="http://www.51xuecheng.cn/course/preview/learning.html?id=${model.course.id}&chapter=${secondNode.teachplanMedia.teachplanId!''}" target="_blank">${secondNode.pname}</a></li>
                                        </#list>
                                    </ul>
                                </div>
                            </div>
    ...
    <div class="popup-course">
        ...
        <div class="popup-course-box">
            <div class="title">${model.course.name} <span class="close-popup-course-box">×</span></div>
            <div class="content">
    ...
    <div class="popup-box">
        ...
        <div class="popup-pay-box">
            <div class="title">${model.course.name} <span class="close-popup-pay-box">×</span></div>
            <div class="content">
                <img :src="qrcode" width="200" height="200" alt="请点击支付宝支付按钮，并完成扫码支付。"/>

                <div class="info">
                    <p class="info-tit">${model.course.name}<span>课程有效期:${model.course.validDays}天</span></p>
                    <p class="info-pic">课程价格 : <span>￥${model.course.originalPrice!''}元</span></p>
                    <p class="info-new-pic">优惠价格 : <span>￥${model.course.price!''}元</span></p>
                </div>
            </div>
            <div class="fact-pic">实际支付: <span>￥${model.course.price!''}元</span></div>
...
<script>var courseId = "${model.course.id}";var courseCharge = "${model.course.charge}"</script>
